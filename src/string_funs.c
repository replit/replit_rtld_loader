...
int itoa(int value, char* sp, int radix) {
  char tmp[34];  // the longest integer's string representation is 32 characters
                 // in base 2. we also need one character for the sign and one
                 // more for the trailing NUL.
  char* tp = tmp;
  int i;
  unsigned v;
  if (radix < 2 || radix > 36)
